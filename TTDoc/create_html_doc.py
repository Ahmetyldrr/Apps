            ...
            <div class="section-card">
                ...
                <div class="section-content">
                    <div class="overview-grid">
                        <div class="stat-box">
                            <h3><i class="fas fa-table me-2"></i>{len(analysis_data)}</h3>
                            <p class="mb-0">Toplam Sekme Sayısı</p>
                        </div>
                        <div class="stat-box">
                            <h3><i class="fas fa-database me-2"></i>{sum(1 for sheet in analysis_data.values() if sheet.get('has_data', False))}</h3>
                            <p class="mb-0">Veri İçeren Sekme</p>
                        </div>
                        <div class="stat-box">
                            <h3><i class="fas fa-rows me-2"></i>{sum(sheet.get('rows', 0) for sheet in analysis_data.values())}</h3>
                            <p class="mb-0">Toplam Satır Sayısı</p>
                        </div>
                        <div class="stat-box">
                            <h3><i class="fas fa-columns me-2"></i>{sum(sheet.get('columns', 0) for sheet in analysis_data.values())}</h3>
...
"""
    ...
    # Her sekme için nav tab oluştur
...
'''
    ...
    for i, (sheet_name, sheet_data) in enumerate(analysis_data.items()):
        ...
        if sheet_data.get('column_names'):
            ...
            columns = sheet_data['column_names'][:12]  # İlk 12 sütunu göster
        ...
        if sheet_data.get('sample_data') and len(sheet_data['sample_data']) > 0:
            html_content += '''
                                    <div class="mt-4">
                                        <h6><i class="fas fa-eye me-2"></i>Veri Önizlemesi (İlk 3 Satır)</h6>
            ...
            # Tablo başlıkları (ilk 6 sütun)
            sample_data = sheet_data['sample_data'][:3]
            if sample_data:
                headers = list(sample_data[0].keys())[:6]
                for header in headers:
                    html_content += f'<th>{header}</th>'
                if len(list(sample_data[0].keys())) > 6:
                    html_content += '<th>...</th>'
        ...
        # Özel notlar
        special_notes = {
            'Teiaş': 'Bu sekme TEİAŞ\'tan alınan ham elektrik iletim verilerini içerir.',
            'TM': 'Trafo merkezi verileri 15 dakikalık aralıklarla kaydedilmiştir.',
            'Nihai': 'Bu sekmede tüm tablolar formüllüdür ve değişiklik yapılmamalıdır.',
            'Dengesizlik': 'Tahmin edilen ve gerçekleşen değerler arasındaki farkları analiz eder.',
            'Endoks': 'Çok sayıda trafo merkezinin detaylı ölçüm verilerini içerir.'
        }
        
        if sheet_name in special_notes:
            html_content += f'''
                                            <div class="alert alert-warning alert-sm mt-3">
                                                <small><i class="fas fa-exclamation-triangle me-1"></i>{special_notes[sheet_name]}</small>
                                            </div>
            '''
    ...
    for i, (sheet_name, sheet_data) in enumerate(analysis_data.items(), 1):
        ...
        description = sheet_descriptions.get(sheet_name, 'Genel veri sekmesi')
    ...
    html_content += f'''
            ...
            <div class="text-center mt-4 mb-3">
                ...
                <p class="text-muted">
                    ...
                    Toplam {len(analysis_data)} Sekme Analiz Edildi</small>
                </p>
            </div>
        </div>
    </div>
